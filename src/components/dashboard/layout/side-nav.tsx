...
export function SideNav(): React.JSX.Element {
  ...
  const filterItems = (items: NavItemConfig[]): NavItemConfig[] => {
    return items.filter(item => {
      // Verificar si el usuario tiene permisos para el item principal
      const hasPermission = !item.requiredPermissions ||
        item.requiredPermissions.some(perm => permissions.includes(perm));

      // Si tiene hijos, filtramos también los hijos
      if (item.children) {
        item.children = filterItems(item.children);
        // Mostramos el item padre solo si tiene hijos visibles
